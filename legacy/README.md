> **[ĐÃ ĐÓNG BĂNG — chỉ để tham khảo]** Đây là bản MVP đầu tiên (SQLite +
> React/Vite). Nền tảng chính đang phát triển tiếp ở `apps/api` (FastAPI +
> Supabase Postgres) và `apps/web` (Next.js) — xem README ở thư mục gốc.
> Logic xử lý `vnstock` đã debug ở đây (fix `source="TCBS"` → `"VCI"`, dò
> tên cột `price_board`...) đã được port sang
> `apps/api/app/collectors/vnstock_adapter.py`.

# Stock Intelligence & Analysis Platform — Full-stack (MVP cũ)
